...
IF exist "%%D" (del /a /f /q /s "%%D\*.*" > nul 2>nul& echo. &&echo SERVER %%A:%%B find %%D
...
@echo.
@echo.
 ) else ( echo. &&echo ERROR - Game log %%D ^do ^not ^find - Edit cfg/servers.cfg)
)
