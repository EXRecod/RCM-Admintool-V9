...
if(empty(SqlDataBase))
{
 ...
 $i_id = $e["num"]; $i_ping = $e["ping"]; $i_ip = $e["ip"]; $i_name = $e["name"]; $i_guid = $e["guid"]; $chistx = $i_name; 
                            ...
                            $x_bann  = explode(" / ", $tk);
                            if ($x_bann[0] == $x_idn) {
                                $db2->query("INSERT INTO bans (playername,ip,guid,reason,time,whooo,patch) VALUES ('$x_bann[1]','$x_bann[2]','','$x_x_m','$datetime','$x_nickx','$game_patch')");
                        ...
                        if ($x_stop_lp == 0)
                          {
                            ...
                            if ($stat > 0)
                              ...
                              {
                                $db2->query("INSERT INTO x_words (z_names,z_words, z_cmdlist) VALUES ('$x_wooord','$x_wooord','$x_reason')");
                                usleep($sleep_rcon * 2);
                                if (($game_patch == 'cod2') || ($game_patch == 'cod4') || ($game_patch == 'cod5'))
