...
if(((!empty($chat_flooder_msg[$conisq][$c]))&&(!empty($chat_flooder_msg[$conisq][$c+1])))
 ||((!empty($chat_flooder_msg[$conisq][$c]))&&(!empty($chat_flooder_msg[$conisq][$c+2])))
 ||((!empty($chat_flooder_msg[$conisq][$c]))&&(!empty($chat_flooder_msg[$conisq][$c+3]))))
{  
...
{
if($zi == 1)
rcon('say  ^6^7  '. $nickr . ' "^2[^7Chat Flooding detected!^2] [^7Warning '.count($chat_flooder_warns[$conisq]).'^2/^7'.chat_s_antiflood_warns.'^2]^7 stop flooding or you get a kick^1!', '');		
else
rcon('say  ^6^7  '. $nickr . ' "^3[^7Chat Flooding detected!^3] [^7Warning '.count($chat_flooder_warns[$conisq]).'^3/^7'.chat_s_antiflood_warns.'^3]^7 stop flooding or you get a kick^1!', '');	
AddToLog("[".$datetime."] CHAT Chat FloodingS KICKER: (" . $nickr . ")");	 	
}
	
if(chat_s_antiflood_warns <= count($chat_flooder_warns[$conisq]))
{
rcon('say  ^6^7  '. $nickr . ' "^1[^7Chat Flooding detected!^1] [^7Warning '.count($chat_flooder_warns[$conisq]).'^1/^7'.chat_s_antiflood_warns.'^1]^7 '.chat_s_antiflood_warns_reason.'^1!', '');	
...
rcon(chat_s_antiflood_warns_reason.' '. $idnum.' CHAT FLOOD!', '');	
else
rcon(chat_s_antiflood_warns_reason.' '. $idnum, '');
