...
if ($mplogfile) {
    if ($x_stop_lp == 0) {
        if (strpos($mplogfile, 'ftp:') !== false) {
            ...
            if (($datedayup == '03') || ($datedayup == '3')) // в 3 часа ночи обнуляем
            {
                ...
                if (filesize($file) > 50000000) {
                    ...
                    if (@ftp_put($conn_idq, $ftp_exp_url, $file, FTP_BINARY)) {
                        ...
                        trigger_error(" RCM DEBUGGER: RESET 50 MB AND RESTART [LOCAL FILE]: $gmlobame");
                        echo "\n FILE $file UPLOADED \n";
                    }
                    else {
                        $cur_activator = 100;
            ...
            if (time() - $dya >= 300) {
                trigger_error("\n RCM Информация: [Не обновлено] за 5 минут не обновило локальный лог игры, перезагрузка мода, обнуление локального лога и кеша.", E_USER_ERROR);
